(() => {
  ...
  const growthDialog=$('#growth-create-dialog'),growthForm=$('#growth-create-form');let growthStep=1,growthExperiment=null,growthCapabilities=[],growthWeekly=null,growthRequests={};
  function growthFields(){return Object.fromEntries(new FormData(growthForm));}
  growthForm.noValidate=true;
  function saveGrowthDraft(){const edited=$('#growth-capabilities').children.length?capabilityFields():growthCapabilities;FS.store('growth-wizard-draft',{...growthFields(),step:growthStep,experiment:growthExperiment,capabilities:edited,weekly:growthWeekly,requests:growthRequests});}
  ...
  function openGrowthWizard(){const draft=FS.load('growth-wizard-draft',{});growthExperiment=draft.experiment||null;growthCapabilities=draft.capabilities||[];growthWeekly=draft.weekly||null;growthRequests=draft.requests||{};if(growthCapabilities.length)renderCapabilities(growthCapabilities);if(growthWeekly)fillWeekly(growthWeekly);for(const [name,value] of Object.entries(draft)){if(growthForm.elements[name]&&typeof value==='string')growthForm.elements[name].value=value;}showGrowthStep(Number(draft.step)||1);growthDialog.showModal();}
  ...
  growthForm.addEventListener('input',saveGrowthDraft);$('#close-growth-create').onclick=()=>growthDialog.close();$('#growth-back').onclick=()=>{if(growthStep===4){delete growthRequests.capabilities;delete growthRequests.week;}showGrowthStep(Math.max(1,growthStep-1));};
  growthForm.addEventListener('submit',async event=>{event.preventDefault();const button=event.submitter;button.disabled=true;$('#growth-wizard-status').textContent='正在准备下一步…';try{const fields=growthFields();
    ...
    if(growthStep===2){if(!fields.desired_outcome?.trim())throw new Error('请填写 12 周后希望留下的真实成果。');if(!growthExperiment){growthRequests.create||=crypto.randomUUID();saveGrowthDraft();const created=await FS.api('/api/growth-experiments',{method:'POST',body:{title:fields.title,future_identity:fields.future_identity,desired_outcome:fields.desired_outcome,request_id:growthRequests.create}});growthExperiment=created.experiment;}else if(fields.title!==growthExperiment.title||fields.future_identity!==growthExperiment.future_identity||fields.desired_outcome!==growthExperiment.desired_outcome){const updated=await FS.api(`/api/growth-experiments/${growthExperiment.id}`,{method:'PATCH',body:{version:growthExperiment.version,title:fields.title,future_identity:fields.future_identity,desired_outcome:fields.desired_outcome}});growthExperiment=updated.experiment;}const draft=await FS.api(`/api/growth-experiments/${growthExperiment.id}/capability-draft`,{method:'POST',body:{}});renderCapabilities(draft.capabilities);showGrowthStep(3);return;}
    if(growthStep===3){growthRequests.capabilities||=crypto.randomUUID();saveGrowthDraft();const confirmed=await FS.api(`/api/growth-experiments/${growthExperiment.id}/capabilities/confirm`,{method:'POST',body:{version:growthExperiment.version,request_id:growthRequests.capabilities,capabilities:capabilityFields()}});growthExperiment=confirmed.experiment;growthCapabilities=confirmed.capabilities;const draft=await FS.api(`/api/growth-experiments/${growthExperiment.id}/weekly-draft`,{method:'POST',body:{}});fillWeekly(draft);showGrowthStep(4);return;}
    const chosen=growthCapabilities.find(item=>item.name===growthWeekly?.action?.capability_name)||growthCapabilities[0];growthRequests.week||=crypto.randomUUID();saveGrowthDraft();const result=await FS.api('/api/weekly-experiments/0',{method:'PATCH',body:{experiment_id:growthExperiment.id,experiment_version:growthExperiment.version,week_number:1,capability_id:chosen.id,hypothesis:fields.hypothesis,success_signal:fields.success_signal,confirm_action:true,request_id:growthRequests.week,action:{title:fields.action_title,first_step:fields.action_first_step,done_criteria:fields.action_done_criteria,planned_minutes:Number(fields.planned_minutes),expected_evidence_kind:growthWeekly?.action?.expected_evidence_kind||'reflection'}}});FS.store('growth-wizard-draft',null);growthDialog.close();await load();FS.toast('成长主题和第一个行动已确认。');if(result.task)location.href='/focus?task='+result.task.id;
