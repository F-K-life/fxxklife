(() => {
  ...
  function paintReview(r,meta={}) {
    ...
    $('review-stats').innerHTML=Object.entries(r.stats||{}).filter(([k,v])=>k in labels&&typeof v==='number').map(([k,v])=>`<div><strong>${v}</strong><span>${labels[k]}</span></div>`).join('');
    const proposal=r.stats?.next_week_proposal,experiment=state?.growth?.active_experiment,capabilities=state?.growth?.capabilities||[],form=$('next-week-form');form.hidden=!proposal||!experiment||Number(r.week_number)>=12;
    if(!form.hidden){const nextAction=proposal.action||{};$('next-week-capability').innerHTML=capabilities.map(item=>`<option value="${item.id}">${escape(item.name)}</option>`).join('');const matched=capabilities.find(item=>item.name===nextAction.capability_name);if(matched)$('next-week-capability').value=matched.id;$('next-week-hypothesis').value=proposal.hypothesis||'';$('next-week-success').value=proposal.success_signal||'';$('next-week-action-title').value=nextAction.title||'';$('next-week-action-step').value=nextAction.first_step||'';$('next-week-action-done').value=nextAction.done_criteria||'';$('next-week-action-minutes').value=nextAction.planned_minutes||25;}open('review-dialog');
  }
  $('next-week-form').onsubmit=e=>{e.preventDefault();action(async()=>{const experiment=state.growth.active_experiment,weekNumber=Number(state.growth.active_week?.week_number||1)+1,key=`next-week-request-${selectedReview}`;let requestId=FS.load(key,null);if(!requestId){requestId=crypto.randomUUID();FS.store(key,requestId);}const result=await FS.api('/api/weekly-experiments/0',{method:'PATCH',body:{experiment_id:experiment.id,experiment_version:experiment.version,week_number:weekNumber,capability_id:Number($('next-week-capability').value),hypothesis:$('next-week-hypothesis').value,success_signal:$('next-week-success').value,confirm_action:true,request_id:requestId,action:{title:$('next-week-action-title').value,first_step:$('next-week-action-step').value,done_criteria:$('next-week-action-done').value,planned_minutes:Number($('next-week-action-minutes').value),expected_evidence_kind:'reflection'}}});FS.store(key,null);$('next-week-form').hidden=true;await refresh();FS.toast('下一周已由你确认。');if(result.task)location.href=`/focus?task=${result.task.id}`;},e.submitter);};
