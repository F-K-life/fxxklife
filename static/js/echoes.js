(() => {
  ...
  function renderMail() {
    ...
    const empty = direction => `<div class="mail-empty">${FS.icon('mail')}<h3>${direction === 'received' ? '让一份真实的行动，成为来信的起点。' : '此刻的你，有什么想对未来说？'}</h3><p>${direction === 'received' ? '一次沉浸后，未来的我会根据真实经历写信。' : '写下一个愿望、一点困惑，或一件想记住的小事。'}</p></div>`;
