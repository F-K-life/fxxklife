(() => {
  ...
  function render() {
    ...
    if (list.length) { const last = list[list.length-1]; $('echo-feature').innerHTML = `<div class="feature-icon">${FS.icon('leaf')}</div><div><span class="eyebrow">YOUR MOST RECENT SMALL STEP</span><h2>${escape(last.title)}</h2><p>${escape(shortDate(last.created_at))} · ${results[last.result] || '真实记录'} · 记录了 ${Math.floor(last.elapsed_seconds/60)} 分钟${last.reflection ? ' · 留下了一句反思' : ''}</p></div><button class="button button-secondary" type="button" data-event="${last.id}">回看这一步${FS.icon('arrow-right')}</button>`; }
    else $('echo-feature').innerHTML = `<div class="feature-icon">${FS.icon('leaf')}</div><div><span class="eyebrow">LITTLE THINGS, REAL PROGRESS</span><h2>第一份回响，从一小步开始。</h2><p>这里不预设你的故事，只收藏你真实走过的路。</p></div><a href="/focus" class="button button-secondary">去沉浸${FS.icon('arrow-right')}</a>`;
  ...
  function showEvent(id) {
    ...
    $('event-feedback-form').onsubmit = ev => { ev.preventDefault(); action(async () => { const data = Object.fromEntries(new FormData(ev.target)); await FS.api('/api/feedback',{method:'POST',body:{event_id:e.id,helpful:data.helpful === 'skip' ? null:data.helpful === 'yes',autonomy:data.autonomy}}); FS.toast('你的感受已保存，不会影响行动记录。'); },ev.submitter); };
    open('event-dialog');
  }
  ...
  async function showLetter(id) {
    ...
    $('letter-direction').textContent = l.locked ? 'SEALED UNTIL YOUR CHOSEN MOMENT' : l.direction === 'received' ? 'A LETTER FROM YOUR FUTURE SELF' : 'A LETTER TO YOUR FUTURE SELF';
  ...
  document.querySelectorAll('[data-mail-tab]').forEach(button => button.onclick = () => { tab = button.dataset.mailTab; document.querySelectorAll('[data-mail-tab]').forEach(b => { b.classList.toggle('active',b === button); b.setAttribute('aria-selected',String(b === button)); }); renderMail(); });
  ...
  $('weekly-review').onclick=e=>action(async()=>{
