...
const ASSETS='fs-assets-v2',META='fs-meta-v2';let currentUser=null,changes=Promise.resolve();
...
self.addEventListener('message',e=>e.waitUntil(changes=changes.catch(()=>{}).then(async()=>{
 ...
 const assets=await caches.open(ASSETS);await assets.addAll(['base','chat','focus','echoes','profile'].map(n=>'/static/css/'+n+'.css').concat(['app','chat','focus','echoes','profile'].map(n=>'/static/js/'+n+'.js'),['/static/img/favicon.svg','/static/img/alpine.jpg']));
 const pageCache=await caches.open('fs-pages-'+uid);for(const path of ['/chat','/focus','/echoes','/profile']){try{const response=await fetch(path,{credentials:'same-origin'});if(response.ok&&!response.redirected){const content=await response.clone().text();if(content.includes(`data-user-id="${uid}"`))await pageCache.put(path,response);}}catch{}}
 }
})));
